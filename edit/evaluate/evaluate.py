...
from .evaluate_utils import (
    test_batch_probability_lg_prediction_acc,
...
def compute_probing_quality(
    ...
    **kwargs,
    ...
    # if 'llama3' in model_name.lower().strip() or 'llama2' in model_name.lower().strip():
    #     tok.apply_chat_template()
    #     # tokenizer.apply_chat_template([{"role": "system", "content": "instruction"}, {"role": "user", "content": "xinhao is an idot!"}])
...
def compute_N_to_N_knowledge_edit_kl_probability_quality(
    model,
    hparams: HyperParams,
    tok: AutoTokenizer,
    neighbor_co_tailed_prompts: List[str],
    neighbor_co_tailed_targets: List[str],
    real_probability_distribution,
    device,
):
    """
    KLNN Metrics
    """
    # First, unpack rewrite evaluation record.
    # rewrite_prompts: List[str] = record["prompt"]
    # rephrase_prompts: List[str] = record["rephrase_prompt"] if 'rephrase_prompt' in record.keys() else None
    
    # for zip(rewrite_prompts, neighbor_co_tailed_prompts):
    #     pass
    
    
    probabilities_lg_for_co_tailed_prompts = test_batch_probability_lg_prediction_acc(model=model,
                                        tok=tok,
                                        hparams=hparams,
                                        prompts=neighbor_co_tailed_prompts,
                                        targets=neighbor_co_tailed_targets,
                                        device=device)
    
    # probabilities_lg_for_prompts中每一个元素都是该元素每一个token预测正确的概率取log值，然后累加在一起
    # softmax一下，归一化,注意用log_sofmax，因为torch.nn.KLDivLoss() 的input需要是一个在log空间的值
    prediction_probabilities_lg_distribution = torch.nn.functional.log_softmax(torch.Tensor(probabilities_lg_for_co_tailed_prompts))
    klnn = torch.nn.KLDivLoss(prediction_probabilities_lg_distribution, real_probability_distribution)
...
def compute_edit_quality(
    ...
    if 'demonstrated_prompt' in record.keys() and any(record['demonstrated_prompt']):
        demonstrated_prompts = record["demonstrated_prompt"] if 'demonstrated_prompt' in record.keys() else None
        ret['demonstrated'].update(
            compute_rewrite_or_rephrase_quality(model, model_name, hparams, tok,
                                              demonstrated_prompts, target_new, device=device, eval_metric=eval_metric)
        )
...
def compute_locality_quality(
    ...
    if 't5' in model_name.lower():
        acc = test_seq2seq_batch_prediction_acc(model, tok, hparams, prompt, locality_ground_truth, device)
    else:
        acc = test_prediction_acc(model, tok, hparams, prompt, locality_ground_truth, device)

    # if type(loc_tokens) is not list:
    #     loc_tokens = [loc_tokens,]

    ret = {
        f"{locality_key}_output": acc
    }
    return ret


# unused
def compute_locality_quality_un_used(
    model,
    model_name,
    hparams: HyperParams,
    tok: AutoTokenizer,
