...
def generate_fast(
    ...
    if 'biogpt' in model.name_or_path.lower():
    ...
    with torch.no_grad():
        ...
        while input_ids.size(1) < max_out_len:  # while not exceeding max output length
            ...
            if 'llama' in model.name_or_path.lower() or 'baichuan' in model.name_or_path.lower():
                model_input_attention_mask = None
            elif 'biogpt' in model.name_or_path.lower():
                entire_context = slice(0, attention_mask.sum(1).min().item())
                model_input_attention_mask = attention_mask[:, entire_context]
            else:
                model_input_attention_mask = attention_mask[:, cur_context]
